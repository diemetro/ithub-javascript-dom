// https://cats.petiteweb.dev/simple/assets/cats.js
const cats = [
    {
        "name": "Лара",
...
catList.style.justifyContent = "center";

// Создаем элементы для отображения информации о котах и устанавливаем значения атрибутов элементов
for (let cat of cats) {
  ...
  if (cat.favourite == true) { 
    catContainer.style.backgroundColor = "bisque";
  } else catContainer.style.backgroundColor = "lightgrey";;

  catContainer.style.borderRadius = "10px";
  catContainer.style.padding = "10px";
  
  const catImg = document.createElement("img");
  catImg.src = cat.img_link;
  catImg.alt = cat.name;
  catImg.style.width = "260px";
  catImg.style.height = "260px";
  catImg.style.objectFit = "cover";
  catImg.style.borderRadius = "10%";
  
  const catName = document.createElement("h2");
  catName.innerText = cat.name;
  catName.style.marginTop = "10px";
  
  const catAge = document.createElement("p");
  catAge.innerText = `Возраст: ${cat.age} лет`;
  
  const catRate = document.createElement("p");
  catRate.innerText = `Оценка: ${cat.rate}`;
...
// Добавляем элементы в контейнер кота
  catContainer.appendChild(catImg);
  catContainer.appendChild(catName);
  catContainer.appendChild(catAge);
  catContainer.appendChild(catRate);
  catContainer.appendChild(catFavourite);
  catContainer.appendChild(catDescription);
  
// Добавляем контейнер кота в список кошек
